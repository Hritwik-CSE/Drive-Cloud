...
export function bindVideoEvents(app) {
  ...
  // Intelligent Fallback: If HTML5 refuses to decode the MKV/AVI/MOV (unsupported codec),
  // instantly punt the stream to the OS native player (VLC/MX Player) instead of freezing.
  videoEl.addEventListener('error', (e) => {
    const err = videoEl.error;
    // MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED (4)
    if (err && err.code === 4) {
      app.showToast('Format unsupported by browser. Attempting to open in native player...');
      ...
      app.showToast('Error loading video playback.');
    }
  });
